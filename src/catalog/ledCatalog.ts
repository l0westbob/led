 ...
 * @returns {string|null}
 */
export function resolveLedSpectralSeriesId(
  led: LedDefinition | undefined | null,
): string | null {
  if (!led) return null;
 ...
 * @returns {string}
 */
export function formatLedDisplayName(led: LedDefinition | undefined | null) {
  if (!led) return "";
  const family = led.family ?? "LED";
  const cct = led.cctK ? `${led.cctK}K` : "Unknown CCT";
  const isMint = String(led.name ?? "")
    .toLowerCase()
    .includes("mint");
 ...
 * @returns {string}
 */
export function resolveBestBinLabel(led: LedDefinition | undefined | null) {
  if (!led) return "n/a";
  const bins = resolveLedBins(led.id);
  const ppfRange = bins?.ppfRangeUmolS ?? led?.reference?.ppfRangeUmolS;
  if (ppfRange && Number.isFinite(ppfRange.maxPpfUmolS)) {
  ...
  if (lmBins.length > 0) {
    ...
    if (
      lmRange &&
      Number.isFinite(lmRange.minLm) &&
      Number.isFinite(lmRange.maxLm)
    ) {
 ...
 */
export function resolveLedDefinition(ledId: string): LedDefinition | null {
 ...
 */
export function listLedDefinitions(): LedDefinition[] {
