...
export class BoardProfile {
  /**
   * @param {Partial<import("@/domain/types").BoardLike>} input
   */
  constructor(input = {}) {
    this.id = input.id ?? "";
  ...
  /**
   * @returns {import("@/domain/types").BoardLike}
   */
  toJSON() {
    return {
      id: this.id,
   ...
   *
   * @returns {Array<{xMm: number, yMm: number, type: string}>}
   */
  generateEmitters() {
...
function sanitizeNumberRange(value, fallback, min, max) {
