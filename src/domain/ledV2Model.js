import {
  ...
  LED_CURVES,
  LED_LIBRARY_V2,
} from "@/domain/LedLibraryV2";
import { estimateBasePpfFromSpectrumCalibration } from "@/domain/spectral/ppfEstimation";
 ...
 * @returns {number}
 */
export function interpolatePiecewiseLinear(points, x) {
 ...
 */
export function estimateLedPpfUmolS(ledId, params) {
  const led = LED_LIBRARY_V2[ledId];
  if (!led) {
    return { ppfUmolS: 0, isEstimated: true, note: "Unknown LED id." };
  }

  const reference = led.reference ?? {};
  const basePpf = Number(reference.ppfUmolS);
  const basePpfFallback = estimateBasePpfFromSpectrumCalibration(led);
  const basePpfEffective = Number.isFinite(basePpf) && basePpf > 0 ? basePpf : basePpfFallback;
  ...
  return {
    ...
    isEstimated:
      (Number.isFinite(basePpf) && basePpf > 0
        ? reference.source !== "datasheet" && reference.source !== "datasheetBinMidpoint"
        : true),
 ...
 * @returns {{ ppfUmolS: number, isEstimated: boolean, note?: string }}
 */
export function estimateBoardPpfUmolS(ledId, params) {
  const led = LED_LIBRARY_V2[ledId];
  if (!led) {
    return { ppfUmolS: 0, isEstimated: true, note: "Unknown LED id." };
  }
  ...
  const basePpf = estimateBasePpfFromSpectrumCalibration(led);
  if (Number.isFinite(basePpf) && basePpf > 0) {
    const calCurrentMA = Number(led.spectrumCalibration?.currentMA ?? 65);
    const perLedV = Number(getPreferredReference(led, { preferHomepageTypical: true })?.forwardVoltageV);
    const calPowerW = (Number.isFinite(calCurrentMA) ? calCurrentMA : 65) / 1000 * (Number.isFinite(perLedV) ? perLedV : 2.7);
...
function estimateSeriesCount(led, boardVoltageV) {
 ...
 */
export function syncBoardElectrical(ledId, params) {
  const led = LED_LIBRARY_V2[ledId];
