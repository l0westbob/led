...
import { LUMINOSITY_V_1NM, LUMINOSITY_V_1NM_START_NM, SPECTRAL_WAVELENGTH_NM } from "./actionCurves";
 ...
 */
export function weightsOnGrid(actionValues, targetWavelengthNm) {
 ...
 */
export function radiantSpdFromLumens(spectrum, lumens) {
  const x = spectrum.wavelengthNm;
  const s = spectrum.intensityRel;
  if (!Array.isArray(x) || !Array.isArray(s) || x.length !== s.length || x.length < 2) {
  ...
  const denom = integrateTrapezoid(x, s.map((vv, i) => (vv ?? 0) * (v[i] ?? 0)));
 ...
 */
export function photonSpdFromRadiant(radiant) {
 ...
 * @returns {{ wavelengthNm:number[], intensityUmolSPerNm:number[] }}
 */
export function photonSpdFromPpf(spectrumRel, ppfUmolS) {
  ...
  const photonRel = s.map((vv, i) => (Number(vv ?? 0) || 0) * (Number(x[i] ?? 0) || 0));
  ...
  return { wavelengthNm: x.slice(), intensityUmolSPerNm: photonRel.map((v) => v * k) };
}
