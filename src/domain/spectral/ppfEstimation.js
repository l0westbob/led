...
import { getSeriesSpectrum } from "./seriesLibrary.js";
import {
  photonSpdFromRadiant,
  radiantSpdFromLumens,
} from "./radiantCalibration.js";
 ...
 * @param {number} nmMax
 */
function integratePhotonRange(photonSpd, nmMin, nmMax) {
...
/**
 * Build a series id (key in `all_series.json`) from an LM301H-family LED object.
 * This matches the naming you use in the digitized dataset.
 */
export function seriesIdForLed(led) {
  ...
  if (family.includes("EVO")) {
    if (String(led?.name ?? "").toUpperCase().includes("MINT")) return "LM301H-EVO-MINT-WHITE";
 ...
 */
export function estimateBasePpfFromSpectrumCalibration(led) {
  const cal = led?.spectrumCalibration;
  if (!cal) return 0;

  if (Number.isFinite(cal.ppfUmolS) && cal.ppfUmolS > 0) return Number(cal.ppfUmolS);
