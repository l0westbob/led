...
export function clampEmitterPosition(input: {
  ...
  const snapStepMm =
    Number(input.snapStepMm) > 0 ? Number(input.snapStepMm) : 1;
  const boardWidthMm = Math.max(0, Number(input.boardWidthMm) || 0);
  const boardDepthMm = Math.max(0, Number(input.boardDepthMm) || 0);
  const snappedX =
    Math.round((Number(input.xMm) || 0) / snapStepMm) * snapStepMm;
  const snappedY =
    Math.round((Number(input.yMm) || 0) / snapStepMm) * snapStepMm;
