  ...
  return input.emitters.map((emitter) => {
    ...
    return {
      ...
      drive: {
        ...
        voltageV: input.roundToTwoDecimals(Number(input.defaultDrive.voltageV) || 0),
        currentA: input.roundToTwoDecimals(Number(input.defaultDrive.currentA) || 0),
...
export function resolveDefaultEmitterDriveDraft(input) {
  ...
  nextDraft.voltageV = input.roundToTwoDecimals(Number(nextDraft.voltageV) || 0);
  nextDraft.currentA = input.roundToTwoDecimals(Number(nextDraft.currentA) || 0);
  return nextDraft;
}
