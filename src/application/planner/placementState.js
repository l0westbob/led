 ...
 * @returns {boolean}
 */
export function doInstanceRectsIntersect(firstInstanceRect, secondInstanceRect) {
...
export function isBoardPlacementValid(input) {
  const { widthCm, depthCm } = resolveBoardInstanceFootprintCm(input.boardInstance);
...
export function findInitialBoardInstancePosition(input) {
  ...
  const gridColumns = Math.max(1, Math.floor(input.roomWidthCm / Math.max(10, input.widthCm + 2)));
  const gridRows = Math.max(1, Math.floor(input.roomDepthCm / Math.max(10, input.depthCm + 2)));
  ...
  for (let rowIndex = 0; rowIndex < gridRows; rowIndex += 1) {
    for (let columnIndex = 0; columnIndex < gridColumns; columnIndex += 1) {
      const xCm = clampToRange(minimumX + columnIndex * stepX, minimumX, maximumX);
...
export function applyBoardInstanceQuarterTurn(input) {
  ...
  const previousRotation = normalizeQuarterTurnRotation(input.boardInstance.rotationDeg);
  input.boardInstance.rotationDeg = normalizeQuarterTurnRotation(previousRotation + deltaDegrees);
  if (
    !isBoardPlacementValid({
