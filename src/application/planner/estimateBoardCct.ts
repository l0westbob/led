...
export type EmitterFluxRow = {
  ledType: string;
  photonFluxUmolS: number;
};
 ...
 */
export function estimateBoardCctFromEmitters(
  emitterFluxRows: EmitterFluxRow[],
) {
  ...
  for (const emitterFluxRow of emitterFluxRows) {
    ...
    const photonFluxUmolS = Math.max(
      0,
      Number(emitterFluxRow.photonFluxUmolS) || 0,
    );
