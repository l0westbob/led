...
const LED_DEFINITION_BY_ID = new Map(KNOWN_LED_DEFINITIONS.map((ledDefinition) => [ledDefinition.id, ledDefinition]));

function resolveReferenceDriveForLed(ledType) {
  const ledDefinition = LED_DEFINITION_BY_ID.get(ledType);
  const referenceCurrentMA = Number(ledDefinition?.reference?.currentMA) || 65;
  const referenceForwardVoltageV = Number(ledDefinition?.reference?.forwardVoltageV) || 2.7;
...
function normalizeEmitterLedType(emitter, fallbackLedType) {
 ...
 */
export function normalizeBoardPlannerEmitters(board) {
      ...
      drive: {
        ...
        driveMode:
          emitterDrive.driveMode === "constantCurrent"
            ? "constantCurrent"
            : "constantVoltage",
        voltageV: Number(emitterDrive.voltageV ?? ledReferenceDrive.voltageV) || ledReferenceDrive.voltageV,
        currentA: Number(emitterDrive.currentA ?? ledReferenceDrive.currentA) || ledReferenceDrive.currentA,
        temperatureC:
          Number(emitterDrive.temperatureC ?? ledReferenceDrive.temperatureC) || ledReferenceDrive.temperatureC,
        seriesCount: 1,
        parallelCount: 1,
      },
    };
 ...
 * @returns {Array<import("@/domain/contracts").BoardEmitter>}
 */
export function generateBoardPlannerEmitterGrid(board) {
 ...
 * @returns {Array<import("@/domain/contracts").BoardEmitter>}
 */
export function regenerateBoardPlannerEmitters(board) {
  ...
  const byPosition = new Map(
    currentEmitters.map((emitter) => [positionKey(emitter.xMm, emitter.yMm), emitter]),
  );
  const byIndex = new Map(currentEmitters.map((emitter, index) => [index, emitter]));

  return nextGridEmitters.map((emitter, index) => {
    const existingEmitter = byPosition.get(positionKey(emitter.xMm, emitter.yMm));
 ...
 * @returns {Array<import("@/domain/contracts").BoardEmitter>}
 */
export function resetBoardPlannerEmittersToDefault(board) {
