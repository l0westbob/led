 ...
 * }}
 */
export function resolveLedContext(input: { ledId: string; stepNm?: 1 | 5 }) {
  ...
  const hasLumenAnchor = Number.isFinite(
    led?.spectrumCalibration?.luminousFluxLm,
  );
  ...
  let spectrum: SpectralSeries | null = null;
  try {
    spectrum = resolveSpectralDataset(led.spectralSeriesId, {
      stepNm: input.stepNm ?? 5,
    });
  } catch {
    warnings.push({
