...
export function buildLedComparisonSnapshot(input) {
  ...
  const activeRows = (Array.isArray(input.comparedLeds) ? input.comparedLeds : []).filter(
    (row) => row.id,
  );
  ...
  const ledSeries = ledSnapshots
    .map(({ snapshot, row }, index) => {
  ...
  const columns = ledSnapshots.map(({ snapshot, row }, index) => {
    const spectrum = snapshot.chartData.series.find((s) => s.id === "led")?.y ?? [];
    const photonFlux = mode === "photon" ? estimatePhotonFluxUmolS(spectrum) : null;
    return {
      key: `led-${index + 1}`,
      label: `LED ${index + 1}`,
      color: row.color,
