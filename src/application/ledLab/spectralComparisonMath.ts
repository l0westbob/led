...
export function estimatePhotonFluxUmolS(photonSeriesValues: number[]) {
 ...
 * @returns {string}
 */
export function formatSpectralMetric(
  spectralStats: Record<string, number | string | null>,
  metricKey: string,
) {
  const value = Number(spectralStats?.[metricKey]);
 ...
 * @returns {number}
 */
export function resolveMaxLedSeriesValue(chartSeries: SpectrumChartSeries[]) {
  let maximumLedValue = 0;
  let maximumAnySeriesValue = 0;
  for (const series of chartSeries) {
    let maximumSeriesValue = 0;
    for (const sampleValue of series.y) {
      maximumSeriesValue = Math.max(
        maximumSeriesValue,
        Number(sampleValue) || 0,
      );
    }
    maximumAnySeriesValue = Math.max(maximumAnySeriesValue, maximumSeriesValue);

    if (!series.id.startsWith("led-")) {
      continue;
    }
    maximumLedValue = Math.max(maximumLedValue, maximumSeriesValue);
  }

  if (maximumLedValue > 0) {
    return maximumLedValue;
  }
  if (maximumAnySeriesValue > 0) {
    return maximumAnySeriesValue;
  }
  return 1;
}
...
export function buildSpectrumSeriesPath(input: {
  ...
  for (
    let sampleIndex = 0;
    sampleIndex < input.seriesValues.length;
    sampleIndex += 1
  ) {
    const wavelengthNm = input.nmMin + sampleIndex;
    const xPosition =
      SPECTRUM_CHART_PADDING +
      ((wavelengthNm - input.nmMin) / nmSpan) * innerWidth;
